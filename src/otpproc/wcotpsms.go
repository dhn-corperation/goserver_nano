...
func smsProcess(wg *sync.WaitGroup) {
	...
	var SMSTable = "OShotSMSOTP_" + monthStr

	//db.Exec("UPDATE OShotSMSOTP SET SendDT=now(), SendResult='6', Telecom='000' WHERE SendResult=1 and date_add(insertdt, interval 6 HOUR) < now()")
	//db.Exec("insert into " + SMSTable + " SELECT * FROM OShotSMSOTP WHERE SendResult>1 AND SendDT is not null and telecom = '000'")
	//db.Exec("delete FROM OShotSMSOTP WHERE SendResult>1 AND SendDT is not null and telecom = '000'")
...
"99"  : "발송시스템 장애 (재전송 필요)",
	...
	if err != nil {
		errlog.Println("스마트미 OTP SMS 조회 중 오류 발생")
		errcode := err.Error()

		if s.Index(errcode, "1146") > 0 {
			db.Exec("Create Table IF NOT EXISTS " + SMSTable + " like OShotSMSOTP")
