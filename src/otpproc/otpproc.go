	...
	for resrows.Next() {
		...
		if len(msg_sms.String) > 0 && len(sms_sender.String) > 0 { // msg_sms 가 와 sms_sender 에 값이 있으면 Oshot 발송 함.
			...
			if len(osmmsStrs) > 500 {
				stmt := fmt.Sprintf("insert into OShotMMSOTP(MsgGroupID,Sender,Receiver,Subject,Msg,ReserveDT,TimeoutDT,SendResult,File_Path1,File_Path2,File_Path3,mst_id,cb_msg_id,userid ) values %s", s.Join(osmmsStrs, ","))
				_, err := db.Exec(stmt, osmmsValues...)

				if err != nil {
					//stdlog.Println("스마트미 SMS Table Insert 처리 중 오류 발생 " + err.Error())
					for i := 0; i < len(osmmsValues); i = i + 13 {
						eQuery := fmt.Sprintf("insert into OShotMMSOTP(MsgGroupID,Sender,Receiver,Subject,Msg,ReserveDT,TimeoutDT,SendResult,File_Path1,File_Path2,File_Path3,mst_id,cb_msg_id,userid ) "+
