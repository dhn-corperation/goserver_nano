...
func mmsProcess(wg *sync.WaitGroup) {
	...
	var MMSTable = "OShotMMSOTP_" + monthStr

	//lms 성공 처리
	//db.Exec("UPDATE OShotMMSOTP SET SendDT=now(), SendResult='6', Telecom='000' WHERE SendResult=1 and date_add(insertdt, interval 6 HOUR) < now()")
	//db.Exec("insert into " + MMSTable + " SELECT * FROM OShotMMSOTP WHERE SendResult>1 AND SendDT is not null and telecom = '000'")
	//db.Exec("delete FROM OShotMMSOTP WHERE SendResult>1 AND SendDT is not null and telecom = '000'")
...
"99"  : "발송시스템 장애 (재전송 필요)",
	...
	if err != nil {
		...
		errcode := err.Error()
		errlog.Println("스마트미 OTP MMS 조회 중 오류 발생", groupQuery, errcode)

		if s.Index(errcode, "1146") > 0 {
			db.Exec("Create Table IF NOT EXISTS " + MMSTable + " like OShotMMSOTP")
...
"99"  : "발송시스템 장애 (재전송 필요)",
	...
	if err != nil {
		...
		errcode := err.Error()
		errlog.Println("스마트미 OTP MMS Pre Month 조회 중 오류 발생", groupQuery, errcode)
